...
test("Portrait constructor defines by-default-undefined constants", () => {
    ...
    expect(typeof pt.RARITY_INDEX).toBe('number');
    expect(typeof pt.ESSENCE_INDEX).toBe('number');
...
test("Portrait methods return undefined if the constructor is empty.", async () => {
    ...
    expect(pt._getPortrait()).toBeUndefined();
});
...
test("method _getPortrait retreives the right portrait based on the name", async () => {
    const pt = new Portrait("BNW", "Cowboy");
    await pt.scrapeData();

    expect(pt._getPortrait()).toMatch(/https:.+?cowboy.+?\.(png|jpg)/i);
});
